                  ...
                  size_t bufferCount)
{
    ...
    if (address != NULL && nxrth_socks5_relay_for ((intptr_t) socket, & relayAddr, & relayLen))
    {
        ...
        if (hdrLen < 0)
          return -1;
        wb [0].buf = (CHAR *) socksHdr;
        wb [0].len = (ULONG) hdrLen;
        for (bi = 0; bi < bufferCount; ++ bi)
        {
            wb [bi + 1].buf = (CHAR *) buffers [bi].data;
            wb [bi + 1].len = (ULONG) buffers [bi].dataLength;
        }
        if (WSASendTo (socket, wb, (DWORD) (bufferCount + 1), & sentLength, 0,
                       (struct sockaddr *) & relayAddr, relayLen, NULL, NULL) == SOCKET_ERROR)
        {
            if (WSAGetLastError () == WSAEWOULDBLOCK)
              return 0;
            return -1;
        }
        /* Report only the ENet payload bytes (exclude the SOCKS5 header). */
        return (int) sentLength - hdrLen;
    }
                     ...
                     size_t bufferCount)
{
    ...
    if (nxrth_socks5_relay_for ((intptr_t) socket, & relayAddr, & relayLen))
    {
        ...
        int srcLen = 0, payloadOff = 0, payloadLen;
        size_t copied = 0, bi;
        int n = recvfrom (socket, (char *) socksBuf, (int) sizeof (socksBuf), 0,
        ...
        {
            size_t take = buffers [bi].dataLength;
            if (take > (size_t) payloadLen - copied)
              take = (size_t) payloadLen - copied;
            memcpy (buffers [bi].data, socksBuf + payloadOff + copied, take);
            copied += take;
        }
        if (address != NULL && src.ss_family == AF_INET)
        {
            struct sockaddr_in * s4 = (struct sockaddr_in *) & src;
            address -> host = (enet_uint32) s4 -> sin_addr.s_addr;
            address -> port = ENET_NET_TO_HOST_16 (s4 -> sin_port);
        }
        return (int) copied;
    }
