...
enet_protocol_send_outgoing_commands (ENetHost * host, ENetEvent * event, int checkForTimeouts)
{
    ...
    /* Growtopia new-packet: compute the 6-byte integrity prefix once per flush.
       The server validates it against the connected peer's port. */
    if (host -> usingNewPacket && host -> peerCount > 0)
    {
        enet_uint16 port = host -> peers [0].address.port;
        enet_uint16 rand1 = (enet_uint16) (enet_host_random (host) % ((enet_uint32) port + 1));
        newHeader -> integrity [0] = ENET_HOST_TO_NET_16 (rand1);
        newHeader -> integrity [1] = ENET_HOST_TO_NET_16 (rand1 ^ port);
        newHeader -> integrity [2] = ENET_HOST_TO_NET_16 ((enet_uint16) ((enet_host_random (host) & 0x61D2) | 0x920D));
    }
         ...
         ++ currentPeer)
    {
        ...
        host -> commandCount = 0;
        host -> bufferCount = 1;
        host -> packetSize = host -> usingNewPacket ? sizeof (ENetNewProtocolHeader) : sizeof (ENetProtocolHeader);
        ...
        if (host -> headerFlags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME)
        {
            if (host -> usingNewPacket)
            {
                ...
                host -> buffers -> dataLength = sizeof (ENetNewProtocolHeader);
            }
            else
            {
          ...
          host -> buffers -> dataLength = host -> usingNewPacket ? ENET_OFFSETOF(ENetNewProtocolHeader, sentTime) : ENET_OFFSETOF(ENetProtocolHeader, sentTime);

        shouldCompress = 0;
        if (host -> compressor.context != NULL && host -> compressor.compress != NULL)
        {
            size_t originalSize = host -> packetSize - (host -> usingNewPacket ? sizeof(ENetNewProtocolHeader) : sizeof(ENetProtocolHeader)),
