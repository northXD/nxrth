/* Bridge between the vendored ENet socket layer (win32.c) and Nxrth's SOCKS5
...
#ifndef NXRTH_SOCKS5_H
#define NXRTH_SOCKS5_H
...
int nxrth_socks5_relay_for(intptr_t socket_handle, struct sockaddr_storage * out_addr,
                            int * out_len);

/* Builds the SOCKS5 UDP request header targeting `target`. Returns header length
   (10 for IPv4) written to `out`, or <0 on error. */
int nxrth_socks5_build_header(const struct sockaddr * target, unsigned char * out, int cap);
