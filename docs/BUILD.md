# Building Adonai (Windows / MSVC / vcpkg)
...
Adonai needs ENet for the game connection. Drop the C ENet source into `third_party/enet/` and add a `CMakeLists.txt` there that builds a static `enet` target — the root CMake auto-detects it (`ADONAI_HAVE_ENET`). Then apply the SOCKS5 relay patch to `enet_socket_send`/`enet_socket_receive` (see `third_party/enet/README.md` and `docs/port-specs/03-net-socks5.md` for the exact header format). Until ENet is vendored the project still configures, but `net/enet_host.*` is compiled with `ADONAI_HAVE_ENET` off (stub) and bots can't connect.
...
```
The exe lands at `build\Release\Adonai.exe`.
...
- the removed HAR/Requestly login modes (Adonai does not use them).
...
`data/`. Proxy config persists to `data/adonai_proxy_pool.json`.
